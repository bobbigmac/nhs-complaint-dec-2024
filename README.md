...
## Progress on making an appointment - Dec 4th 2025
  ...
  - Tests all came back pretty much normal (was not tested for calprotectin, despite asking for that test), health condition in NHS app was closed on jan 6th, with no follow-up. The app doesn't say which doctor flagged the condition I am trying to identify as PAST, but I consider this an oversight, as the cause of my condition was not identified, only things that are not the cause. 
    - Also was not tested for celiac, so not sure why I was told I would be.
    - Patient was not tested for the test that would probably show the problem, then case closed without confirmation from the patient or thorough check about the current status.
    - Process issue or lack of care?
    - This same issue was raised by another patient in the Nov 26th PPG. Likely, the process/workflow encourages closing off conditions or open tickets, but doesn't recognise if the patient was diagnosed/treated, only that the tests were done. 'The system' check what it was testing for, closed the condition anyway.
  ...
  - Woke up monday evening to 2 missed calls (so much for that text message) at 10:44 and 11:33. Obviously missed them because I told the receptionist I would be asleep and she told me she'd send a text message.
    - I'm genuinely baffled by this. I saw the receptionist write a post-it about asking Lynne and sending me a text message, but that somehow got translated into a regular 2 phone calls (missing both, obviously), but with no entry created on the system (so when I went to follow-up, no record). I want to know how this happened, surely the presence of a 'send text message' post-it either _should_ create a record in the system with that as a function, or it should not, and would be handled manually.
      - It looks like somebody decided to take an explicit agreement offered by reception (they would send me a text) which could work, and downgraded it to a contact method they KNOW can NOT result in an appointment. 
