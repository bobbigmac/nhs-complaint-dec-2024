...
function renderGtdSurveyTrendChart(svg, summary, legend, heading, note) {{
  ...
  if (note) note.textContent = 'Thin lines show each GTD practice\\'s GP Patient Survey overall-experience-as-good percentage by year. Faint dashed vertical lines mark the documented GTD takeover date. The first legend entry restores the default GTD mean plus New Bank view; when a named practice is pinned, the GTD mean is hidden. The green dashed line shows registered patients as a percentage of the GTD-wide average patient count for that year, with raw patient counts kept in the point labels. Data from gp-patient.co.uk practice-level CSV.';
  const width = 920;
  const height = 360;
  ...
  const hasPinnedSpecificPractice = Boolean(pinnedTrendPracticeCode) && pinnedTrendPracticeCode !== TREND_DEFAULT_CONTEXT_CODE && availableCodes.has(pinnedTrendPracticeCode);
  const showAverage = !hasPinnedSpecificPractice;
