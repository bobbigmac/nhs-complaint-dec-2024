...
function renderDeprivationChart() {{
  ...
  const distributionMarkup = gridX.map((decile) => {{
    ...
    const alpha = columnPoints.length ? (0.03 + (diffStrength * 0.14)) : 0.02;
    const bandAlpha = columnPoints.length ? (0.12 + (diffStrength * 0.22)) : 0.05;
    const isNeutral = shiftStrength < 0.12;
    const fill = isNeutral
      ? `rgba(120, 128, 138, ${{alpha.toFixed(3)}})`
      : shift >= 0
        ? `rgba(28, 124, 84, ${{(alpha + shiftStrength * 0.05).toFixed(3)}})`
        : `rgba(195, 71, 47, ${{(alpha + shiftStrength * 0.05).toFixed(3)}})`;
    const bandFill = isNeutral
      ? `rgba(120, 128, 138, ${{bandAlpha.toFixed(3)}})`
      : shift >= 0
        ? `rgba(28, 124, 84, ${{(bandAlpha + shiftStrength * 0.08).toFixed(3)}})`
        : `rgba(195, 71, 47, ${{(bandAlpha + shiftStrength * 0.08).toFixed(3)}})`;
    ...
    const title = !columnPoints.length
      ...
      : `IMD decile ${{decile}}: distribution is ${{isNeutral ? 'similar to' : shift >= 0 ? 'better than' : 'worse than'}} the overall pattern. Jensen-Shannon divergence ${{jsd.toFixed(2)}}.`;
    return `
      <g>
        <rect x="${{x0.toFixed(2)}}" y="${{margin.top.toFixed(2)}}" width="${{(x1 - x0).toFixed(2)}}" height="${{plotHeight.toFixed(2)}}" fill="${{fill}}">
          <title>${{title}}</title>
        </rect>
        <rect x="${{(x0 + 1).toFixed(2)}}" y="${{margin.top.toFixed(2)}}" width="${{Math.max(0, x1 - x0 - 2).toFixed(2)}}" height="7" rx="3.5" fill="${{bandFill}}">
          <title>${{title}}</title>
        </rect>
      </g>
    `;
