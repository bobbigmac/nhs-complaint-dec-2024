...
This folder is a small watchlist plus operator-profile source file for non-GTD operators and similar groups that keep coming up around the Greater Manchester work.
...
- keep a compact source-of-truth profile layer for the future org chart / org navigator page
- avoid burying that knowledge inside one enrichment script

## Files

- `watchlist.json` - manually curated operator profiles, tender notes and the specific practices or lots we want to keep checking.
- `build_company_watchlist.py` - reads the current enriched dataset and writes a watchlist report.
- `output/company_watchlist_report.md` - human-readable summary of known operators plus which watchlist companies are already in the current catchment dataset.
...
- Some profiles are intentionally marked as dataset-led / thinner than others. That is preferable to silently dropping a meaningful operator cluster from the navigator.
